"""
Non-linearly separable case SVM, no kernels, and optimization problem is solved by "cvxpy" library
I am minimizing the squared norm2 of the w vector plus a pondered sum of epsilon vector with respect
to w and b and subject to the constraints that each training example must have functional margin >= 1-epsilon.
Sometimes the quadratic solver won't find solutions for certain values of c and a relatively small change has to 
be applied to c.
"""
...
class SVM:
    ...
    def fit(self, c = 1.5):
        ...
        b = cvxpy.Variable()
        epsilons = cvxpy.Variable((len_data, 1))

        objective = cvxpy.Minimize(cvxpy.sum_squares(w) + c * cvxpy.sum(epsilons))

        constraints = [y_values[i][0] * (X_values[i, :].reshape(1,-1) @ w + b) >= (1 - epsilons[i][0]) for i in range(len_data)]
        epsilons_constraints = [epsilons[i][0] >= 0 for i in range(len_data)]

        constraints += epsilons_constraints
